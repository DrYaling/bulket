...
void UAMC_MovementComponent::TickComponent(float DeltaTime, ELevelTick TickType,
...
{
	float MaxMoveAcceleration = Super::GetMaxAcceleration();
	int count = 0;
	if (bSprintEnabled == true && bWantsToSprint == true)
	{
		...
		{
			MaxMoveAcceleration *= SprintAccelerationMultiplierCurve->GetFloatValue(SprintTimeHeldDown);
		}
		else
		{
			MaxMoveAcceleration *= SprintAccelerationMultiplier;
		}
	}
	if (bJetpackEnabled == true && bWantsToJetpack == true)
	{
		...
		{
			MaxMoveAcceleration *= JetpackAccelerationMultiplierCurve->GetFloatValue(JetpackTimeHeldDown);
		}
		else
		{
			MaxMoveAcceleration *= JetpackAccelerationMultiplier;
		}
	}
	if (count > 0)
	{
		MaxMoveAcceleration /= count;
	}
	return MaxMoveAcceleration;
}

void UAMC_MovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector & OldLocation, const FVector & OldVelocity)
{
	if (!CharacterOwner || bFreezing)
	{
		return;
	}
...
bool UAMC_MovementComponent::CanJump()
{
	...
	return IsMovingOnGround() && CanEverJump();
}
...
void UAMC_MovementComponent::SetJetpacking(bool IsJetpacking)
{
	bWantsToJetpack = IsJetpacking;
}
...
void UAMC_MovementComponent::SetSprinting(bool IsSprinting)
{
...
void UAMC_MovementComponent::SetFreeze(float Duration, bool OverrideDuration)
{
	if (Duration <= 0.0)
		return;
	bFreezing = true;
	if (OverrideDuration)
		...
		FreezingDuration += Duration;
	if (const auto RCharacter = Cast<ARustCharacter>(CharacterOwner))
	{
		RCharacter->GetAnimInstance()->SetFreeze(Duration, OverrideDuration);
	}
}

void UAMC_MovementComponent::SetNotFreeze()
{
	bFreezing = false;
	FreezingDuration = 0.0;
	if (const auto RCharacter = Cast<ARustCharacter>(CharacterOwner))
	{
		RCharacter->GetAnimInstance()->SetNotFreeze();
	};
}
