...
UnrealBindings CreateBindings()
{
	...
	actor_fns.set_location_rotation = &SetActorLocationAndRotation;
	UnrealBindings b = {};
...
#endif
#if WITH_EDITOR
		//if game is running ,quit game
		if (const auto SGame = sGameInstance) {
			UKismetSystemLibrary::QuitGame(SGame->GetWorld(), nullptr, TEnumAsByte<EQuitPreference::Type>(EQuitPreference::Quit), true);
		}
#endif
	}
}
...
bool FPlugin::TryLoad()
{
	...
	FPlatformFileManager::Get().GetPlatformFile().CopyFile(*LocalTargetPath.Replace(TEXT(".dll"), TEXT(".pdb")), *Path.Replace(TEXT(".dll"), TEXT(".pdb")));
#endif
